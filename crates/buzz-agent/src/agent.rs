...
impl RunCtx<'_> {
        ...
        loop {
            ...
            if response.tool_calls.is_empty() {
                ...
                if stop == StopReason::EndTurn {
                    ...
                    // Final steer drain before a direct-answer turn ends. The
                    // round-boundary drain at the top of the loop only runs
                    // when there IS a next round — a plain text turn returns
                    // here with none, so a steer accepted mid-call (the client
                    // already got its ok ack) would never be picked up and
                    // would die silently with the run. Accepted input must be
                    // consumed inside the turn: fold it in and run one more
                    // round; only an empty queue truly ends the turn. The loop
                    // converges because this drain is the sole reason to
                    // continue, and it yields nothing on re-entry.
    ...
    /// Returns true when at least one renderable steer was folded in — the
    /// EndTurn drain uses this to decide whether the turn must continue.
    ...
    async fn execute_calls(&mut self, calls: &[ToolCall]) -> Option<StopReason> {
        ...
        for (idx, call) in calls.iter().enumerate() {
            ...
            if call.name == builtin::LOAD_SKILL_TOOL {
                ...
                let mut result =
                    builtin::call_load_skill(&call.arguments, self.skills, Some(self.mcp.policy()))
                        .await;
