...
| service 如何双读 / 谁是单写者 | **不双读**：启动时一次性迁移（`src/task_migrate.rs`）后由 task worker 独占执行；定义单写者 = CLI（原子落盘）；运行态**只由 service 内的 task worker 写**（Q12），但 2026-09-26 起同 bot 可有多个 worker 并发写 → 一律走 `TaskStateStore::update_if`（单锁内判定+落盘） |
...
| `cron` | **上次记账之后应到期的那个命中分钟点已过** | **逾期认领**（见下方）；**同一分钟只触发一次**（`last_fired_at` 分钟桶去重；worker 每 2s 轮询，无此记账会反复触发）；新登记（无记账）不追溯历史周期 |
...
> **cron 逾期认领（2026-09-26 修订，线程 `abb-task-worker-cron-parallel-20260926`）**：改前 cron 判据是
> 「**当前分钟**必须命中表达式」，于是该 bot 的 task worker 被任何一条长任务占住时，落在那个分钟点上的
> 触发**永久消失**——不执行、不重试、不补跑、`last_fired_at` 不变、**日志一行都没有**（线程现场实测：
> `*/10` 的巡检与另一条单轮 7–14 分钟的任务共用同一 worker，实际退化成 10–30 分钟一次）。
> 现改为按 [`src/task_run.rs::cron_due`] 判定：
> - 有 `last_fired_at` 时，判据放宽为「上次记账之后存在一个**已到期**的命中分钟点」→ 被占住而错过的
>   那个点会在 worker 空出来后**补跑一次**（认领即刷新 `last_fired_at`，所以不会重放整段历史）；
>   只补最近一次：跨越多个错过的分钟点也只认领一轮。
> - **扫描窗口 48h**（`CRON_CATCHUP_WINDOW_SECS`）：更早的错过点不追溯；周期长于窗口的任务
>   （如每年 1 月 1 日）在非命中时刻不会因为「很久以前有一轮」被拉起来。
> - 无 `last_fired_at`（新登记）**保持原语义**：当前分钟命中才跑，不追溯登记之前的历史周期。
> - 记账在未来（时钟回拨/手改状态文件）→ 不信这笔记账，回落为当前分钟匹配（与 `interval` 同口径）。
> - **可观测性**：认领时刻比应到时刻晚 ≥ 60s 时，日志记一行
>   `逾期认领：应到 <本地时刻>，迟到 <N>s（执行位此前被占）`——「定时任务为什么没跑」不再是静默的。
> - **与 Q5 的边界**：service 停机期间错过的 cron 点，只要还在同一 48h 窗口内，启动后也会按上述规则
>   补跑一次（这是本修订的**有意**结果：「到点就该执行」；Q5 的「不补历史周期」约束的是 keepalive/proc
>   的恢复语义，不是这条）。
>
...
> （在定时档通道内排在该档队列里，每条各打一行逾期日志）——重启后短时间内出现多行「逾期认领」属预期，不是故障。
...
| Q7 | **执行容量**：每 bot 几个 task worker？task 与聊天谁优先？超限排队还是拒绝？ | **B1：task 用独立 handle/pool**（不与聊天共用单 slot）→ task 与聊天互不阻塞。B2（harness 多 slot）暂不做，留作后续扩容路径。**超限排队**（不拒绝）。**2026-09-26 修订：每 bot 两条执行通道**——定时档（`once`/`cron`/`interval`）与手动档（`now`/`keepalive`），**通道内**超限排队（同档可配 N 个 worker 并发跑**不同**任务）、**跨通道互不阻塞**（否则 now 档的长任务会把定时触发饿死）；**同通道 worker 数可配**（`config.json#task_workers`，默认 `schedule: 2 / manual: 1`，0 按 1 兜底）——定时档默认并行 2 条，到点即执行；**同一条任务永不自我重叠**（认领是单锁 check-and-set），并发只作用于**不同**任务 |
...
| Q14 | 进程退出/信号/补跑语义 | 默认宽限 **10s**（`limits.grace_secs` 可覆盖）→ **SIGKILL**；**对进程组 / Job Object 整体发信号**。补跑：once 补 1 次；cron 逾期认领补「最近一次被占住而错过的分钟点」（48h 窗口内，2026-09-26 修订，见 P2b-C 一节）；interval 按 `last_fired_at + 间隔` 判定、迟到一轮即补；三者都**不重放整段历史周期** |
