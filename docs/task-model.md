...
> 判断「现在能不能停」请看 #309、§4 与代码，不要据本章下结论。
...
- ⇒ **复盘时 job 不能可靠被停止词打断**；这是现存缺陷、不只是设计问题，当时单开了 **#309**。
...
| **P2a** | task store + CLI + 权限/身份模型 | D4 | 中 | 待开工 |
| **P2b** | task channel + 执行容量方案 + cancel/GC/workspace 接入 | D1a/b/c、Q7 | **高**（动共享执行层） | 待开工 |
