...
- 任务执行并发（Q7「上限可配」）：`config.json#task_workers` = `{"schedule":2,"manual":1}`（默认；每通道取值 1–8，0 按 1 兜底、超上限截断并在日志告警）。定时档（`once`/`cron`/`interval`）默认 **2** —— 到点即执行，不再因为同档另一条长任务排队；手动档（`now` 后台子代理 / `keepalive`）默认 **1**。同一条任务**永远不会**自我重叠（认领是单锁 check-and-set）；调大的代价是同时跑的真模型回合变多、且未显式指定 `workspace` 的任务会共用 bot 工作区。
