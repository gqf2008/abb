...
| 跨会话投递 | 设置里开启后，agent 可把消息/任务结果投递到其它 bot 的会话（`agent-bridge deliver --bot <key> --chat <id> --text ...`）；附件由接收端平台**真发送**（飞书全类型；微信图片/文件/视频，音频按文件发；钉钉当前仅群聊图片。不支持的类型显式报错，不再只发本地路径文本）；支持定时任务多目标（`job add --to bot:chat`） |
