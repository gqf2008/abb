...
/// #64 路由转发 Messenger：kind=github bot 的 Bridge.msgr（GitHub 不是 IM 通道，没有
/// 自己的发送端——所有通知/提及/自动处理回执经它转发到目标 IM bot 的会话）。
/// send_text 解析 "bot_key:chat_id"（split_once，chat 段可含 ':'）；裸 chat_id 或目标
/// bot 不在表 → Err（走 watch 既有重试语义，目标修好后自愈，不静默丢）。
pub struct RoutedMessenger {
    targets: HashMap<String, std::sync::Arc<dyn Messenger>>,
}

impl RoutedMessenger {
    pub fn new(targets: HashMap<String, std::sync::Arc<dyn Messenger>>) -> RoutedMessenger {
        RoutedMessenger { targets }
    }

    /// "bot_key:chat_id" → (bot_key, chat_id)；chat 段可含 ':'。无 ':' → None。
    fn parse_target(chat: &str) -> Option<(&str, &str)> {
        chat.split_once(':')
    }
}

#[async_trait::async_trait]
impl Messenger for RoutedMessenger {
    async fn send_text(&self, chat_id: &str, text: &str) -> Result<()> {
        let Some((bot_key, chat)) = Self::parse_target(chat_id) else {
            anyhow::bail!("GitHub 通知目标需 bot_key:chat_id 格式（收到裸 chat_id「{chat_id}」）");
        };
        let Some(msgr) = self.targets.get(bot_key) else {
            anyhow::bail!("通知目标 bot「{bot_key}」不存在或未启用");
        };
        msgr.send_text(chat, text).await
    }
    // send_thread_reply 用默认实现（→ 调 self.send_text，同样解析转发）；
    // typing/del_typing/done/get_quoted_message/download_attachment/user_display_name
    // 全部默认 no-op（github bot 无表情/无附件下载/无用户反查，正确）。
}
...
mod tests {
    ...
    #[test]
    fn routed_parse_target_shapes() {
        assert_eq!(RoutedMessenger::parse_target("b:c"), Some(("b", "c")));
        assert_eq!(
            RoutedMessenger::parse_target("b:c:d"),
            Some(("b", "c:d")),
            "chat 段可含冒号"
        );
        assert_eq!(RoutedMessenger::parse_target("bare"), None);
        assert_eq!(RoutedMessenger::parse_target(""), None);
    }

    #[tokio::test]
    async fn routed_forwards_to_target_bot() {
        let fake = std::sync::Arc::new(FakeMsgr {
        ...
        targets.insert(
            "im".to_string(),
            fake.clone() as std::sync::Arc<dyn Messenger>,
        );
        let r = RoutedMessenger::new(targets);
        r.send_text("im:oc_1", "hello").await.unwrap();
        assert_eq!(
            fake.sent.lock().unwrap()[0],
            ("oc_1".to_string(), "hello".to_string())
        );
    }

    #[tokio::test]
    async fn routed_errs_on_bare_and_unknown_bot() {
        let r = RoutedMessenger::new(HashMap::new());
        let e1 = r.send_text("bare", "x").await.unwrap_err().to_string();
        assert!(e1.contains("bot_key:chat_id"), "裸格式报格式提示: {e1}");
        let e2 = r
            .send_text("ghost:oc_1", "x")
            .await
            .unwrap_err()
            .to_string();
        assert!(e2.contains("ghost"), "未知 bot 报目标名: {e2}");
    }
}
