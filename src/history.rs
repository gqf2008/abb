...
//!   或 claude 换 UUID 自愈后 pending 标记放行一次注入），或 pi 会话文件丢失（#56：
//!   pi 对不存在文件同 sid 静默新建，无错误可检——run 前探查直接注入）——/new、CLI
//!   reset 使 marker 失效或失配，无需额外清理路径。CLI `session reset` 不清历史
//!   （与 /new 不对称，有意：reset 后注入续命恰是「会话丢失自愈」的目标语义）。
