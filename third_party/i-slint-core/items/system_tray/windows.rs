...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    ICONINFO, MF_DISABLED, MF_GRAYED, MF_POPUP, MF_SEPARATOR, MF_STRING,
...
impl PlatformTray {
    pub fn new(
        ...
        //
        // 本地补丁（#111）：父窗口从 `HWND_MESSAGE`（message-only）改为 `None`
        // （顶层隐藏窗口）。explorer 重启时 `TaskbarCreated` 经 HWND_BROADCAST
        // 只广播给顶层窗口，message-only 窗口收不到 → 图标重建逻辑永不触发。
        // 顶层隐藏窗口不可见（无 WS_VISIBLE），不影响任务栏/Alt-Tab，仅用于
        // 接收系统广播，行为与 message-only 在其余场景等价。
